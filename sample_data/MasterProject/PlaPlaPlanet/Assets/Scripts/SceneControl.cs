	...
	void	OnGUI()
	{
		...
		case STEP.RESULT:
			...
			if(flag_clear){
				...
				GUI.color = Color.black;
				// GUI.Label(new Rect(pos_x,pos_y,200,20), "脱出" + this.clear_time.ToString("0.00"), guistyle);

				int time = (int)clear_time;
				if(time >50){
					GUI.Label(new Rect(pos_x, result_rect.y +240, 200,20), "ぎりぎり脱出おめでとう！");	break;
				}else if(time >40){
					GUI.Label(new Rect(pos_x, result_rect.y +240, 200,20), "脱出おめでとう！");	break;
				}else if(time >30){
					GUI.Label(new Rect(pos_x, result_rect.y +240, 200,20), "もうすぐ！ぷらぷらマスター！"); 		break;
				}else{
					GUI.Label(new Rect(pos_x, result_rect.y +240, 200,20), "ぷらぷらマスター！"); 		break;
				}
				...
				// GUI.color = Color.black;
				// GUI.Label(new Rect(pos_x,pos_y,200,20), "ゲームオーバー", guistyle);
			}
			break;
		}
	}
